...
Make sure you are either logged into twitter as the account you plan to use, or not logged in. Click the authorize link and log-in, then click "Allow". Twitter will then present you with a PIN number. B<To complete your connection to tircd>, type: /stats pin <PIN NUMBER>. On some clients you may have to type: /quote stats pin <PIN NUMBER>. 
...
* Next time you connect, tircd will first check to see if a password is saved in your F<Username.config>, and comapre it with the one your client sends.
...
* If you want to change your password, or did not set one on your first connection, you can do so after you have connected to tircd with a simple C<< /STATS password <password> >>
...
* If you want to remove the password, simply issue a C</STATS password> without any password set.
